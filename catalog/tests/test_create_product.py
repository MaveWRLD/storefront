...
class TestCreateProduct:
    def test_anonymous_cannot_create_product(self, collection):
        ...
        response = client.post(
            '/store/products/', product_payload(collection), format='json')
    ...
    def test_missing_title_returns_400(self, admin_client, collection):
        ...
        del payload['title']
        response = admin_client.post('/store/products/', payload, format='json')
    ...
    def test_admin_can_attach_an_image_to_a_product(self, admin_client, collection):
        ...
        response = admin_client.post(
            f'/store/products/{product_id}/images/',
            {'image': buf, 'alt_text': 'A shirt'},
            format='multipart')

        assert response.status_code == status.HTTP_201_CREATED

        detail = admin_client.get(f'/store/products/{product_id}/')
    ...
    def test_non_admin_cannot_attach_image(self, admin_client, collection):
        ...
        client = APIClient()
        response = client.post(
            f'/store/products/{product_id}/images/', {'alt_text': 'nope'})
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
